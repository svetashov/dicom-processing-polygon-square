...
def complete_selection(image):
    if len(points) > 2:
        ...
        cv2.putText(image, 'S=' + '{0:.4g}'.format(cm_square) + 'cm^2', (10, 40), cv2.FONT_HERSHEY_SIMPLEX, .7, (255, 0, 255))
...
if __name__ == '__main__':
    ...
    voi_lut_img = dicom.pixel_data_handlers.util.apply_voi_lut(dimage.pixel_array, dimage, index=0)
    # extract density
    if dimage.get('PixelSpacing') != None:
        pixel_spacing = dimage.get('PixelSpacing')
    array = voi_lut_img.astype(float)
